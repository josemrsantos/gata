...
| `linkedin_post.md` | Researched, non-satirical companion article — independently researched by Claude/Gemini/Grok, opening with a labelled Executive Summary and organised by agreed angles, with a code-built Sources list; the AI-authorship disclosure and pipeline metrics sit at the bottom, inside "Behind the Scenes" (`--linkedin-post` only) |
