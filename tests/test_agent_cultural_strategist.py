...
from agents.types import ConversationLog, EnrichedBrief, LoopOutput, StrategyBrief
...
def test_run_returns_tuple_of_enriched_brief_and_log():
    # run() must return a 2-tuple so pipeline.py can unpack the EnrichedBrief and
    # ConversationLog for bundle writing without any extra call.
    with patch("agents.agent_cultural_strategist.DualPersonaLoop") as MockLoop:
        MockLoop.return_value.run.return_value = _LOOP_OUTPUT
        result = run(_TOPIC, _SEED)
...
def test_run_first_element_is_enriched_brief():
    ...
    # `enriched_brief, log = agent_0.run(...)` without index lookups.
    with patch("agents.agent_cultural_strategist.DualPersonaLoop") as MockLoop:
        MockLoop.return_value.run.return_value = _LOOP_OUTPUT
        enriched_brief, _ = run(_TOPIC, _SEED)
    assert isinstance(enriched_brief, EnrichedBrief)
